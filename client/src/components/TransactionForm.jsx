...
export default function TransactionForm({ onTransactionAdded, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onTransactionAdded(); // Tell dashboard to refetch
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div>
        ...
        <div className="relative mt-1 rounded-md shadow-sm">
          ...
          <input
            ...
            placeholder="0.00"
            step="0.01"
            className="block w-full rounded-md border-gray-300 pl-8 pr-4 py-2 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-black"
          />
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 mt-5">
        <button
          type="button"
          onClick={onClose}
          className="py-2 px-4 bg-gray-200 text-gray-800 font-medium rounded-md shadow-sm hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2"
        >
          Cancel
        </button>
        <button
          type="submit"
          className="py-2 px-6 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold rounded-md shadow-md hover:from-blue-600 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
        >
